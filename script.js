...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const refreshHelper = () => {
    ...
    const shouldDisableNext = totalSelections === 0;
    if (subjectsButton) {
      subjectsButton.disabled = shouldDisableNext;
    }
    if (nextButton) {
      nextButton.disabled = shouldDisableNext;
    }
  };
  ...
  if (subjectsButton) {
    subjectsButton.addEventListener("click", () => {
      if (subjectsButton.disabled) return;
      goToSubjects();
    });
  }
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const refreshHelper = () => {
    ...
    if (nextButton) {
      nextButton.disabled = disableCheckout;
    }
  };
